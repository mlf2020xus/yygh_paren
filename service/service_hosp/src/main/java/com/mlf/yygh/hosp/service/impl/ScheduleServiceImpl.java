...
import com.mlf.yygh.hosp.mapper.ScheduleMapper;
import com.mlf.yygh.hosp.repository.ScheduleRepository;
...
public class ScheduleServiceImpl extends ServiceImpl<ScheduleMapper,Schedule> implements ScheduleService {
    ...
    //做聚合、分组、排序、统计使用MongoTemplate会更加灵活
    @Autowired
    ...
    public Map<String, Object> getRuleSchedule(long page, long limit, String hoscode, String depcode) {
        //1、根据医院编号 和 科室编号 查询
        /**  mongoTemplate里面有一个封装条件的对象 Criteria **/
        Criteria criteria = Criteria.where("hoscode").is(hoscode).and("depcode").is(depcode);
        //2、根据mongoDB里面的workDate日期进行分组   Aggregation是mongoTemplate做聚合操作的对象
        Aggregation agg = Aggregation.newAggregation(
                Aggregation.match(criteria),//匹配的条件
                Aggregation.group("workDate")//分组字段
                .first("workDate").as("workDate")//为这个字段添加一个别名
                //3、统计号源数量
                .count().as("docCount")//就诊医生人数
                .sum("reservedNumber").as("reservedNumber")//科室可预约数
                .sum("availableNumber").as("availableNumber"),//科室剩余预约数
                //再做一个排序  根据workDate做排序
        ...
        List<BookingScheduleRuleVo> bookingScheduleRuleVoList = aggResults.getMappedResults();//返回日期数据的集合
    ...
    public Map<String, Object> getBookingScheduleRule(Integer page, Integer limit, String hoscode, String depcode) {
        ...
        //3、获取可预约日期里面科室的剩余预约数  Criteria是mongoDB的条件封装对象  Aggregation对聚合条件封装对象
        Criteria criteria = Criteria.where("hoscode").is(hoscode).and("depcode").is(depcode)
        ...
        //合并数据  map集合 key表示日期  value表示预约规则和剩余数量等
        Map<Date, BookingScheduleRuleVo> scheduleVoMap = new HashMap<>();
        if(!CollectionUtils.isEmpty(scheduleVoList)) {
            scheduleVoMap = scheduleVoList.stream().
                    collect(//Collectors.toMap()表示转成map集合
    ...
    public ScheduleOrderVo getScheduleOrderVo(String scheduleId) {
        ...
        DateTime quitTime = this.getDateTime(new DateTime(schedule.getWorkDate())
                .plusDays(quitDay).toDate(), bookingRule.getQuitTime());
